...
| `TT_JOIN_CODE`                      | `paddle`             | Legacy setting; registrations now use admin review |
| `ADMIN_USERNAME` / `ADMIN_PASSWORD` | `admin` / `changeme` | Hidden staff desk                                  |

## How it works

- **Players:** first name + last name. No email, no login. New registrations remain hidden until approved at `/admin`;
  after approval, the browser that registered can edit or remove **only that listing**.
- **Matches:** single elimination, best of 3, games to 11, win by 2 from 10–10. The tournament administrator records
  results and advances winners.
