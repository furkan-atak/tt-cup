...
export async function POST(request: Request) {
  ...
  const expectedCode = process.env.TT_JOIN_CODE;
  if (expectedCode && parsed.data.joinCode !== expectedCode) {
    return jsonError("Join code is not valid.");
  }

  const claimedId = await getClaimedPlayerId();
  if (claimedId) {
